...
export class DrizzleUserRepository implements UserRepository {
  async create(user: UserEntity): Promise<UserEntity> {
    try {
      ...
      throw new IntegrationError(`Failed to create user: ${(error as Error).message}`)
    }
  }
  ...
  async update(user: UserEntity): Promise<UserEntity> {
    try {
      ...
      throw new IntegrationError(`Failed to update user: ${(error as Error).message}`)
    }
  }
  ...
  async getByUsername(username: string): Promise<UserEntity | null> {
    try {
      ...
      throw new IntegrationError(`Failed to get user by username: ${(error as Error).message}`)
    }
  }
  ...
  async getByEmail(email: string): Promise<UserEntity | null> {
    try {
      ...
      throw new IntegrationError(`Failed to get user by email: ${(error as Error).message}`)
    }
  }
